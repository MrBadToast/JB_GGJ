...
#if UNITY_EDITOR
using UnityEditor;
[CustomEditor(typeof(Object_Pool))]

public class Object_Pool_Editor : Editor
{
...
public class Object_Pool : MonoBehaviour
{
	...
	public static GameObject SpawnFromPool(string tag, Vector3 position) => inst._SpawnFromPool(tag, position, Quaternion.identity);

	public static GameObject SpawnFromPool(string tag, Vector3 position, Quaternion rotation) => inst._SpawnFromPool(tag, position, rotation);
	...
	{
		if (!poolDictionary.ContainsKey(tag)) { throw new Exception($"Pool with tag {tag} doesn't exist."); }
